...
- **`adjusted_close` is computed, not fetched**: `compute_adjusted_close` in `data_getter.py` runs after the per-ticker pulls and writes `adjusted_close` into each DataFrame. The per-ticker math is `data_loader.dividend_adjust(close, dividends)` — dividend-only, because yfinance's close is already split-adjusted. `tests/test_adjustment.py` cross-checks against yfinance's native `Adj Close` on a small sample (<1e-4 relative error).
...
- **[`doc/TODO.md`](doc/TODO.md)** — known limitations ranked by impact on backtest believability. The top three (HPO multiple-testing inflation, CV autocorrelation leakage, missing spread/slippage in cost model) can each erase or invert apparent edge; survivorship bias and code-level TODOs sit below them. Cite the relevant caveats when reporting yields/Sharpe/Sortino, not just survivorship.
